...
public class Drawboard extends JPanel {
    ...
    public void savePanelAsPPT(String folder, String fileName) {
        if (fileName == null || fileName.trim().isEmpty()) {
            JOptionPane.showMessageDialog(null, "文件名不能为空，保存失败...");
            return;
        }

        // 自动补全扩展名，如果没有指定扩展名，默认保存为 .png
        ...
        try {
            ImageIO.write(image, extension, file);
            JOptionPane.showMessageDialog(null, "图像已成功保存到当前路径：" + file.getAbsolutePath());
            EventListener.getInstance().clear(true);
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "保存失败，发生异常...");
        }
    }
